...
import { runScoringResearch } from "../src/server/logic/scoring.js";
import { computeProfitabilityScore } from "../src/server/logic/profitabilityEngine.js";
import { buildFinalScores } from "../src/server/logic/scoringEngine.js";
import { calculateValuation } from "../src/server/logic/valuation.js";
import { generateReportNarrative } from "../src/server/logic/reportNarrative.js";
import type {
...
/**
 * Vercel serverless function for full analysis chain
 */
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    console.log(
      `[${timestamp}] [ANALYZE] Request received:`,
      JSON.stringify(
        {
          website,
          revenue,
    ...
    const research: ScoringResearchOutput = await runScoringResearch(revenue, grossProfit, websiteExtraction);

    // Backend profitability math
    const profitability = computeProfitabilityScore(revenue, grossProfit, research.sgnaBand.mid);
    console.log(`[${timestamp}] [PROFITABILITY] Metrics:`, JSON.stringify(profitability, null, 2));
